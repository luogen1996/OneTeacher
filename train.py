...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    if opt.imagenet_weights is not None:
        logging.warning('load imagenet weights!')
    ...
    # with open('/hdd1/zyy/code/datasets/VOC/val.txt','w') as fil:
    #     for _ in f:
    #         fil.write(_.replace('../datasets/VOC','.')+'\n')
    # Trainloader
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if RANK in [-1, 0] and epoch % opt.val_per_epoch==0:
            # mAP
            ...
            if not noval or final_epoch:  # Calculate mAP
                results, maps, _ = val.run(data_dict,
                                           ...
                                           imgsz=imgsz,
                                           model=deepcopy(model) if ema is None else ema.ema,
...
def parse_opt(known=False):
    ...
    parser.add_argument('--step-lr', action='store_true', help='linear LR')
