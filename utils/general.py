...
def non_max_suppression_pseudo_decouple_multi_view(prediction, conf_thres=0.2,cls_thres=0.2, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
                        labels=None, max_det=300,multi_view_thres=0.2,multi_view_iou_thres=0.7):
    ...
    t = time.time()
    output_cls = [torch.zeros((0, 6), device=prediction[0].device)] * prediction[0].shape[0]
    output_reg = [torch.zeros((0, 6), device=prediction[0].device)] * prediction[0].shape[0]
    for xi, views in enumerate(zip(*prediction)):  # image index, image inference
        ...
        for view in views[1:]:
            ...
            i_ = torchvision.ops.nms(xywh2xyxy(view_[:, :4]), view_[:, 4], iou_thres)  # NMS
            view_=view_[i_]
            ...
            view_item[(view_score < multi_view_iou_thres)] *= 0.
            view_scores.append(view_item.unsqueeze(-1))
            view_masks.append(((view_score >= multi_view_iou_thres)).unsqueeze(-1).unsqueeze(-1).float())
       ...
       '''
        ...
        output_cls[xi] =output_boxes[(output_boxes[:,4]>conf_thres) & (output_boxes[:,5]>cls_thres)][:,:6]
...
def non_max_suppression_pseudo_decouple(prediction, conf_thres=0.25,cls_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
                        labels=None, max_det=300):
    """Runs Non-Maximum Suppression (NMS) on inference results
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
       ...
       '''
        ...
        if labels is not None and len(labels):
            ...
            l = labels[(labels[:, 0] == xi).nonzero().squeeze(1)][:, 1:]
            mask=torch.cat([mask,torch.ones(len(l),2,dtype=mask.dtype,device=mask.device)],0)
            if len(l):
                # Cat apriori labels if autolabelling
                v = torch.zeros((len(l), nc + 5), device=x.device)
                v[:, :4] = l[:, 1:5]  # box
        ...
        if multi_label:
            i, j = (x[:, 5:] > conf_thres).nonzero(as_tuple=False).T
            x = torch.cat((box[i], x[i, j + 5, None], j[:, None].float(),mask[i].float()), 1)
        else:  # best class only
            conf, j = x[:, 5:].max(1, keepdim=True)
            x = torch.cat((box, conf, j.float(),mask.float()), 1)[conf.view(-1) > conf_thres]
        ...
        output_x=x[i]
        output_reg[xi] =  torch.masked_select(output_x[:,:6], output_x[...,-1].unsqueeze(1).bool()).view(-1, 6)
        outbox =  torch.masked_select(output_x[:,:6], output_x[...,-2].unsqueeze(1).bool()).view(-1, 6)
